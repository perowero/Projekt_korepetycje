...
export const ListUnpaidStudent=({onMakePayment})=>{
    const [paids,setPaids]=useState([]);
    const token = localStorage.getItem('token');

    useEffect(()=>{
        fetch("http://localhost:8080/api/lessons/private-unpayments", {
            headers:{
      ...
      {paids.length === 0 ? (
        ...
        paids.map((paid) => (
          ...
          >
            ...
            <button onClick={()=>onMakePayment(paid.id)}>zaplac</button>
          </div>
        ))
      )}
    </div>
